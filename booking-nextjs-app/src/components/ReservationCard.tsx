...
export default function ReservationCard({
  ...
  const { fromCity, toCity, company, timeFrom, dateFrom, bookingDate, price } =
  ...
  const handleComplete = async (e: SyntheticEvent) => {
    ...
    await fetch(`/api/booking-success-page`, {
      method: "PATCH",
      body: JSON.stringify({
      ...
      <div className="flex flex-col gap-3.5 md:gap-4 bg-white p-4 md:p-6 shadow-[0_4px_8px_rgba(0,0,0,0.15)] rounded-lg">
        <div className="grid grid-cols-2 grid-rows-2 md:grid-rows-1 md:grid-cols-[3fr_3fr_1fr] gap-y-1 md:gap-y-0">
          <p className="md:text-2xl text-gray-800 font-bold">
            {reservationData.id.split("-")[0]}
          </p>
          <p className="md:text-2xl text-gray-800 font-bold md:font-normal justify-self-end md:justify-self-start">
            {bookingDate}
          </p>
          <p className="text-green-500 md:text-2xl md:justify-self-end font-bold md:font-normal">
            {price} ₽
          </p>
        </div>
        <Separator />
            ...
            <div className="flex flex-col gap-3.5 md:gap-4">
              ...
              <div>
                <p className="text-xs md:text-sm max-w-[700px] text-gray-800">
                  Бесплатная отмена бронирования возможна до 10 декабря 2024,
                  23:59.
                </p>
                <p className="text-xs md:text-sm max-w-[700px] text-gray-800">
              ...
              <div className="flex flex-col md:flex-row gap-2">
                ...
                <button
                  type="button"
                  className="border border-gray-300 px-4 py-2 rounded-lg text-gray-800 font-bold hover:border-transparent text-sm md:text-bases"
                >
        ...
        <div className="flex gap-2 self-center md:self-start cursor-pointer" onClick={handleOpenClose}>
          <p className="text-blue-500 text-sm md:text-base">{isOpen ? "Свернуть" : "Показать полностью"}</p>
          <Arrow className={`[&_path]:stroke-blue-500 ${isOpen ? 'rotate-90' : '-rotate-90'} w-5 h-2.5 self-center`} />
        </div>
      </div>
    </form>
  );
